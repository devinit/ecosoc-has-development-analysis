...
fit = lm(usd_disbursement_crs~usd_disbursement_iati+recipient_name+humanitarian, data=dat_train)
summary(fit)
...
abline(0,1)
original_mse = mean(sqrt((dat_test$usd_disbursement_crs - dat_test$usd_disbursement_iati)^2))
original_mse
plot(usd_disbursement_crs~usd_disbursement_iati_fit, data=dat_test)
abline(0,1)
fit_mse = mean(sqrt((dat_test$usd_disbursement_crs - dat_test$usd_disbursement_iati_fit)^2))
fit_mse
